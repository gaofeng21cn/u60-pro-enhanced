...
3. USB 数据线直接连接电脑，设备保持亮屏；拔下 USB 网卡，电脑保留其他可用网络用于下载依赖。首次安装保持现有 USB/ADB 组合；设备默认使用原厂 RNDIS。Mac 直连请在安装完成后使用 Release 包的 `macos/u60-rndis.sh`，不要手动切换 USB 组合。不要远程跨 Tailscale 执行首次安装。
...
可下载本仓库 [v0.1.10-experimental 安装包](https://github.com/gaofeng21cn/u60-pro-enhanced/releases/tag/v0.1.10-experimental)，下载用户不需要编译器。以下是开发者构建方式。构建依赖 Zig 0.14.1、Go、Python 3 和 Git，主机测试还需 C 编译器和 Node.js。**请显式使用 Zig 0.14.1**：0.16 会构建失败。
...
```sh
cd dist/u60-pro-enhanced-v0.1.10-experimental
python3 prepare.py
```
