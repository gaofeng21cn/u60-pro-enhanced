...
''');curl.chmod(0o700)
 ...
 def run():return subprocess.run(['sh',str(p/'setup.sh'),'--enable-ntp'],env=dict(os.environ,PATH=str(p/'bin')+':'+os.environ['PATH'],TEST_ROOT=td),capture_output=True,text=True)
 (p/'config.yaml').write_text(old);r=run();assert r.returncode==0,r.stderr
 cfg=(p/'config.yaml').read_text();assert cfg.startswith(old) and 'write-to-system: false' in cfg and 'dialer-proxy: DIRECT' in cfg
